...
export default function LoginPage() {
  ...
  return (
    <div
      ...
      style={{
        background:
          'radial-gradient(1200px 600px at 10% -10%, #1a1d23 0%, transparent 60%), radial-gradient(900px 500px at 110% 110%, #2a1d12 0%, transparent 55%), #0f1115',
      }}
    >
      {/* grid bg */}
      <div
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-6 sm:mb-8 flex flex-col items-center">
          ...
          <img
            ...
            alt="EEST Logo"
            className="w-44 sm:w-56 mb-4 sm:mb-5 mx-auto drop-shadow-[0_0_24px_rgba(255,122,26,0.15)] invert brightness-0"
          ...
          <p className="text-zinc-500 text-xs sm:text-sm mt-1.5 tracking-wide">
            Maintenance Report · v2.0
          </p>
        </div>
        ...
        <div className="bg-zinc-900/80 backdrop-blur border border-zinc-800 rounded-sm p-6 sm:p-8 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.6)]">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-sm font-medium text-zinc-300 tracking-[0.2em] uppercase">Login</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-[11px] font-medium text-zinc-500 mb-2 tracking-wider uppercase">Email</label>
              <input
            ...
            <div>
              <label className="block text-[11px] font-medium text-zinc-500 mb-2 tracking-wider uppercase">Password</label>
              <input
          ...
          <div className="mt-6 border-t border-zinc-800 pt-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-zinc-500 text-[11px] tracking-wider uppercase">Test Accounts</p>
              <span className="text-[10px] font-mono text-zinc-600">password: 1234</span>
            </div>
            <div className="space-y-1">
              {testUsers.length === 0 ? (
                <p className="text-zinc-600 text-xs animate-pulse">กำลังโหลด...</p>
              ) : (
                testUsers.map(u => (
                  <div
                    key={u.email}
                  ...
                  >
                    <div className="flex flex-col gap-0.5 min-w-0">
                      <span className="text-zinc-200 group-hover:text-orange-400 transition truncate font-medium">
                        {u.fullname}
                      </span>
                      <span className="text-zinc-500 truncate text-[11px] font-mono">{u.email}</span>
                    </div>
                    <div className="flex flex-col items-end gap-0.5 shrink-0 ml-3">
                      <span className={`font-semibold uppercase tracking-wider text-[10px] ${roleColor[u.role?.toLowerCase()] ?? 'text-zinc-500'}`}>
                        {u.role}
                      </span>
                      {u.section && (
                        <span className="text-zinc-500 text-[10px] font-mono">{u.section}</span>
                      )}
                    </div>
                  </div>
                ))
            ...
            <p className="text-zinc-600 text-[11px] mt-3">▸ แตะที่แถวเพื่อกรอกอัตโนมัติ</p>
          </div>
        </div>
